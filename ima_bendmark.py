...
with driver.session(database='binary-ima') as session:
    # Initialize timing variables with default values
    t_query_a1 = 0
    t_query_a2 = 0
    t_query_b = 0
    t_query_c = 0
    ...
    # ----------------- Read Radiologists Report Excel file -----------------
    print("Reading Radiologists Report Excel file...")
    try:
        radiologists_report = pd.read_excel("Radiologists Report.xlsx")
        print(f"Read {len(radiologists_report)} radiologist notes ✅")
        # Create a dictionary for quick lookup by patient_id
        radiologists_notes = {}
        for _, row in radiologists_report.iterrows():
            try:
                patient_id = str(row['Patient ID']).zfill(4)  # Ensure 4-digit format with leading zeros
                note_text = str(row["Clinician's Notes"]) if pd.notna(row["Clinician's Notes"]) else ""
                radiologists_notes[patient_id] = note_text
            except Exception as e:
                print(f"Error processing row in Excel file: {e}")
        print(f"Processed {len(radiologists_notes)} unique patient notes ✅")
    except Exception as e:
        print(f"Error reading Radiologists Report Excel file: {e}")
        radiologists_notes = {}

    # After the batch insert is complete, add the Note nodes
    if radiologists_notes:
        print("Adding radiologist notes to database...")
        notes_batch = []
        for patient_id, note_text in radiologists_notes.items():
            notes_batch.append({
        ...
        try:
            session.run("""
                UNWIND $batch AS record
                MATCH (p:Patient {patient_id: record.patient_id})
                CREATE (n:Note {text: record.note_text, created_date: datetime()})
                CREATE (p)-[:HAS_NOTE]->(n)
            """, batch=notes_batch)
            print(f"Added {len(notes_batch)} notes to database ✅")
        except Exception as e:
            print(f"Error adding notes to database: {e}")

        # Check if notes were actually created
        try:
            ...
            note_count = record["note_count"] if record else 0
            print(f"Verified {note_count} notes in database")
        except Exception as e:
    ...
    try:
        ...
        print(f"Error in Use Case A1: {e}")
        t_query_a1 = 0
        res_a1 = []
    ...
    try:
        ...
        print(f"Error in Use Case A2: {e}")
        t_query_a2 = 0
        res_a2 = []
    ...
    try:
        ...
        print(f"Error in Use Case E: {e}")
        t_query_e = 0
        res_e = []

    # ----------------- Use Case F: Query Patients with Notes -----------------
    if radiologists_notes:
        try:
            start_time = time.time()
            result_f = session.run("""
                MATCH (p:Patient)-[:HAS_NOTE]->(n:Note)
                RETURN p.patient_id as patient_id, n.text as note_text
                LIMIT 10
            """)
            res_f = [dict(record) for record in result_f]
            t_query_f = time.time() - start_time
            
            print(f"Found {len(res_f)} patients with notes")
        except Exception as e:
            print(f"Error in Use Case F: {e}")
            t_query_f = 0
            res_f = []
    else:
        print("No radiologist notes found, skipping Use Case F")
        t_query_f = 0
        res_f = []
    ...
    try:
        ...
        print(f"Error in sequential queries benchmark: {e}")
    ...
    try:
        ...
        print(f"Error in base64 statistics: {e}")
...
results.append({
    ...
    "UseCaseF_NotesQuery_Time": t_query_f,
    "Records": len(data_list),
    ...
    "UseCaseF_Records": len(res_f)
})
print(f"Neo4j: Insert {t_insert:.2f}s | A:SimpleLookup {t_query_a1+t_query_a2:.3f}s | B:Relationship {t_query_b:.3f}s | C:Aggregation {t_query_c:.3f}s | D:ComplexFilter {t_query_d:.3f}s | E:Base64Query {t_query_e:.3f}s | F:NotesQuery {t_query_f:.3f}s")
print("\n===========================================")
